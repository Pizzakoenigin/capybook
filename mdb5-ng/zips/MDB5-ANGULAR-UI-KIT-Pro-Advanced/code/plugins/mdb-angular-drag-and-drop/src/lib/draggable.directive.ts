...
export class MdbDraggableDirective implements OnInit, OnDestroy {
  ...
  @Output() dragStart: EventEmitter<HTMLElement> = new EventEmitter<HTMLElement>();
  @Output() dragEnd: EventEmitter<HTMLElement> = new EventEmitter<HTMLElement>();
  ...
  private _handleDragMove(event: any): void {
    ...
    if (!this._isMoving) {
      ...
      if (this._pickUpPosition && isOverThreshold) {
        ...
        this._ngZone.run(() => {
          this._isMoving = true;
          this.dragStart.emit(event.target);
        });
  ...
  private _handleDragEnd(event: any): void {
    ...
    this._ngZone.run(() => {
      ...
      this.dragEnd.emit(event.target);
    });
  }
