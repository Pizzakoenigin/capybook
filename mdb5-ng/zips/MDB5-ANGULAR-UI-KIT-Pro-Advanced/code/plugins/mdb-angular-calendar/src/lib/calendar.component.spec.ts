...
class DefaultCalendarComponent {
  @ViewChild(MdbCalendarComponent) calendar: MdbCalendarComponent;
...
describe('Mdb Calendar', () => {
  ...
  describe('onClick', () => {
    ...
    let element: any;
    let component: any;
    let calendar: MdbCalendarComponent;
    ...
    it('should change view after using public method changeView', fakeAsync(() => {
      fixture.detectChanges();
      const selectInput: HTMLInputElement = document.querySelector('.select-input');

      expect(calendar.view).toBe('month');
      expect(selectInput.value).toBe('Month');

      calendar.changeView('list');
      fixture.detectChanges();

      expect(calendar.view).toBe('list');
      expect(selectInput.value).toBe('List');

      calendar.changeView('week');
      fixture.detectChanges();

      expect(calendar.view).toBe('week');
      expect(selectInput.value).toBe('Week');
    }));
  ...
  describe('MDB Calendar Inputs', () => {
    ...
    it('should not render navigation when navigation input is set false', fakeAsync(() => {
      component.navigation = false;
      fixture.detectChanges();
      flush();
      const calendarTools = fixture.nativeElement.querySelector('.calendar-tools');
      const navigationDiv = calendarTools.querySelector(
        '.my-2.me-2.my-lg-0.d-flex.justify-content-center'
      ) as HTMLDivElement;
      expect(navigationDiv).toBeFalsy();
    }));

    it('should not render view select when viewSelect input is set false', fakeAsync(() => {
      component.viewSelect = false;
      fixture.detectChanges();
      flush();
      const calendarTools = fixture.nativeElement.querySelector(
        '.calendar-tools'
      ) as HTMLDivElement;
      const viewSelectForm = calendarTools.querySelector('mdb-form-control');
      expect(viewSelectForm).toBeFalsy();
    }));

    it('should not render add event button when addEventButton input is set false', fakeAsync(() => {
      component.addEventButton = false;
      fixture.detectChanges();
      flush();
      const calendarTools = fixture.nativeElement.querySelector(
        '.calendar-tools'
      ) as HTMLDivElement;
      const addButton = calendarTools.querySelector('.btn.btn-primary') as HTMLButtonElement;
      expect(addButton).toBeFalsy();
    }));

    it('should not render tooltips when tooltips input is set to false', fakeAsync(() => {
      directive = fixture.debugElement
        .query(By.directive(MdbTooltipDirective))
        .injector.get(MdbTooltipDirective) as MdbTooltipDirective;
      const monthTableElement = fixture.nativeElement.querySelector('.month') as HTMLTableElement;
      const eventElement = monthTableElement.querySelector('.event') as HTMLDivElement;
      const onShown = jest.spyOn(directive.tooltipShown, 'emit');
      eventElement.dispatchEvent(new Event('mouseenter'));
      fixture.detectChanges();
      flush();
