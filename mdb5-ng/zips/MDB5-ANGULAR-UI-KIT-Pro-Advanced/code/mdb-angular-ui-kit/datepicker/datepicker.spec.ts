...
describe('MDB Datepicker', () => {
  ...
  describe('Opening and closing', () => {
    ...
    it('should open and close component after date selection if confirmDateOnSelect input is set to true', fakeAsync(() => {
      datepicker.confirmDateOnSelect = true;
      fixture.detectChanges();
      datepicker.open();
      fixture.detectChanges();
      flush();
    ...
    it('should not render Cancel button if removeCancelBtn input is set to true', fakeAsync(() => {
      datepicker.removeCancelBtn = true;
      fixture.detectChanges();
      datepicker.open();
      fixture.detectChanges();
      flush();

      const cancelButton = document.querySelector('.datepicker-cancel-btn') as HTMLButtonElement;

      expect(datepicker._isOpen).toBe(true);
      expect(cancelButton).toBeNull();
    }));

    it('should not render Clear button if removeClearBtn input is set to true', fakeAsync(() => {
      datepicker.removeClearBtn = true;
      fixture.detectChanges();
      datepicker.open();
      fixture.detectChanges();
      flush();

      const clearButton = document.querySelector('.datepicker-clear-btn') as HTMLButtonElement;

      expect(datepicker._isOpen).toBe(true);
      expect(clearButton).toBeNull();
    }));

    it('should not render Ok button if removeOkBtn input is set to true', fakeAsync(() => {
      datepicker.removeOkBtn = true;
      fixture.detectChanges();
      datepicker.open();
      fixture.detectChanges();
      flush();

      const okButton = document.querySelector('.datepicker-ok-btn') as HTMLButtonElement;

      expect(datepicker._isOpen).toBe(true);
      expect(okButton).toBeNull();
    }));

    it('should not open component on toggle click if datepicker disabled input is set to true', fakeAsync(() => {
    ...
    it('should not open component on toggle click if datepicker toggle disabled input is set to true', fakeAsync(() => {
      datepicker._toggle.disabled = true;
      fixture.detectChanges();
      flush();
      const toggleButton = fixture.nativeElement.querySelector('.datepicker-toggle-button');
      toggleButton.click();
  ...
  describe('rendering custom icon', () => {
    it('should render custom icon if icon input is set', fakeAsync(() => {
      basicComponent.icon = 'fab fa-angular';
      fixture.detectChanges();
      flush();

      const iconElement = document.querySelector('.datepicker-toggle-icon') as HTMLLIElement;
    ...
    it('should render custom icon if template icon is provided', fakeAsync(() => {
      basicComponent.iconTemplate = true;
      fixture.detectChanges();
      flush();
...
describe('MDB CustomHeaderDatepicker', () => {
  let overlayContainer: OverlayContainer;
  let overlayContainerElement: HTMLElement;
  let fixture: ComponentFixture<CustomHeaderDatepicker>;
  let customHeaderComponent: CustomHeaderDatepicker;
  let input: HTMLInputElement;
  let datepicker: MdbDatepickerComponent;
  ...
  beforeEach(() => {
    fixture = createComponent(CustomHeaderDatepicker);
    fixture.detectChanges();
    customHeaderComponent = fixture.componentInstance;
    datepicker = fixture.componentInstance.datepicker;
    input = fixture.debugElement.query(By.css('input')).nativeElement;
  });
  ...
  it('should render custom header template', fakeAsync(() => {
    ...
    const headerTitleText = document.querySelector('.datepicker-title-text') as HTMLSpanElement;
    expect(headerTitleText.textContent).toBe('Choose date');
  }));
});
    ...
    <mdb-form-control>
      ...
      <mdb-datepicker-toggle
        *ngIf="!iconTemplate"
        ...
        [disabled]="disabled"
      ></mdb-datepicker-toggle>
      <mdb-datepicker-toggle *ngIf="iconTemplate" [mdbDatepicker]="basicDatepicker"
        ><ng-template mdbDatepickerToggleIcon
          ><i class="fa-angular fab datepicker-toggle-icon"></i></ng-template
      ></mdb-datepicker-toggle>
      <mdb-datepicker
        [inline]="inline"
        [startDate]="startDate"
...
@Component({
  selector: 'mdb-custom-header-datepicker',
  template: `
    <mdb-form-control>
      <input
        mdbInput
        [mdbDatepicker]="customHeaderDatepicker"
        type="text"
        class="form-control"
        id="exampleDatepicker1"
      />
      <label mdbLabel for="exampleDatepicker1" class="form-label">Select a date</label>
      <mdb-datepicker-toggle [mdbDatepicker]="customHeaderDatepicker"></mdb-datepicker-toggle>
      <mdb-datepicker #customHeaderDatepicker>
          ...
          <div class="datepicker-header">
            ...
            <div class="datepicker-date">
              <span class="datepicker-date-text"
                >{{ customHeaderDatepicker.activeDay }}/{{
                  customHeaderDatepicker.activeMonth + 1
                }}/{{ customHeaderDatepicker.activeYear }}</span
              >
            </div>
          </div></ng-template
        >
