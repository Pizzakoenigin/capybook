...
export class MdbAutocompleteDirective implements OnDestroy, ControlValueAccessor {
  ...
  private _handleKeyDown(event: KeyboardEvent) {
    ...
    if (this.mdbAutocomplete) {
      ...
      if (key === HOME) {
        if (this.activeOption && this._isDropdownOpen) {
  ...
  private _resetHighlight() {
    if (this.activeOption) {
      this.mdbAutocomplete._keyManager.setActiveItem(null);
    }
  }
  ...
  writeValue(value: any): void {
    Promise.resolve().then(() => {
      const displayValue =
        this.mdbAutocomplete && this.mdbAutocomplete.displayValue
          ? this.mdbAutocomplete.displayValue(value)
          : value;

      const newValue = displayValue != null ? displayValue : '';
