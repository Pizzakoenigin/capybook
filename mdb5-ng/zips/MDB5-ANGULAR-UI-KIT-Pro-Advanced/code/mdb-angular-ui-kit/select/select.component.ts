...
export type MdbSelectFilterFn = (option: string, filterValue: string) => boolean;
  ...
  implements OnInit, OnDestroy, AfterContentInit, ControlValueAccessor
{
  ...
  @Input() filterFn: MdbSelectFilterFn = (option, filterValue) => option.includes(filterValue);
        ...
        this.selectFilter.valueChanges
          ...
          .subscribe((value: string) => {
            ...
            const options = this.options.toArray().filter((option: MdbOptionComponent) => {
              if (this.filterFn(option.label.toLowerCase().trim(), filterValue)) {
                option.hidden = false;
                return option;
              } else {
                option.hidden = true;
